...
an unwind handler) is recorded on that step's operation record, shows
up in `inv.Failure().UnwindFailures` for the unwind steps after it, and
...
terminates with `OutcomeFailure` and the root failure's attribution.
