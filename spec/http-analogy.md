...
| wrapping one handler at mux registration, `mux.Handle(pattern, mw(h))` | pipeline-level middleware: `pipelinedef.Config.Middleware`, set with the generated package's `WithMiddleware` option on the constructor; composes inside the engine chain |
...
| `http.Server` / `Server.Shutdown` | `engine.Engine` / `Engine.Stop` |
