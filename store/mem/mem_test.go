...
func TestGetActiveRunIDFollowsTheSlot(t *testing.T) {
	...
	s := mem.New()
	rec := func(id durable.RunID, group string) *driver.RunRecord {
		return &driver.RunRecord{RunID: id, PipelineID: "p", ResourceID: "r", Group: group, Phase: durable.PhaseForward, CreatedAt: time.Unix(1, 0), UpdatedAt: time.Unix(1, 0)}
	}
	if _, ok, err := s.GetActiveRunID(ctx, "pipeline/p", "r"); err != nil || ok {
		t.Fatalf("empty store: ok=%v err=%v", ok, err)
	}
	if _, created, err := s.CreateRun(ctx, rec("a", "")); err != nil || !created {
		t.Fatal(err)
	}
	if id, ok, _ := s.GetActiveRunID(ctx, "pipeline/p", "r"); !ok || id != "a" {
		t.Fatalf("after create: %q %v", id, ok)
	}
	if _, ok, _ := s.GetActiveRunID(ctx, "group/other", "r"); ok {
		t.Fatal("another group's slot must be free")
	}
	if _, created, _ := s.CreateRun(ctx, rec("b", "")); created {
	...
	if _, ok, _ := s.GetActiveRunID(ctx, "pipeline/p", "r"); ok {
	...
	if id, ok, _ := s.GetActiveRunID(ctx, "pipeline/p", "r"); !ok || id != "c" {
